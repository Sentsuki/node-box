...
// hasSubscriptionBracket 判断字符串是否包含形如 "[xxx]" 的订阅名前缀模式
func hasSubscriptionBracket(s string) bool {
	start := strings.Index(s, "[")
	if start == -1 {
		return false
	}
	end := strings.Index(s[start:], "]")
	return end > 1 // 确保 [] 之间至少有一个字符
}

// CleanAllSubscriptionArtifacts removes ALL nodes (outbounds and endpoints) and selector tags
// that contain the subscription bracket pattern "[xxx]" in their tags.
// Unlike CleanSubscriptionArtifacts which only removes known subscription names,
// this method removes any content that looks like it was generated by a subscription,
// ensuring that nodes from renamed/deleted subscriptions are also cleaned up.
func (u *Updater) CleanAllSubscriptionArtifacts(configPath string) error {
	data, err := os.ReadFile(configPath)
	if err != nil {
		return fmt.Errorf("%w %s: %v", ErrConfigFileRead, configPath, err)
	}
	...
	for _, outboundRaw := range outboundsArray {
		...
		if tag, ok := outboundMap["tag"].(string); ok && tag != "" {
			if hasSubscriptionBracket(tag) {
				removedCount++
				continue
			}
		}

		// 清理 selector/urltest 的 outbounds 列表
		if t, ok := outboundMap["type"].(string); ok && (t == "selector" || t == "urltest") {
			if obList, ok := outboundMap["outbounds"].([]any); ok {
				var filtered []any
				for _, ob := range obList {
					if s, ok := ob.(string); ok && hasSubscriptionBracket(s) {
						continue
					}
					filtered = append(filtered, ob)
				}
	...
	// 清理 endpoints（relay 节点可能被写入此处）
	removedFromEndpoints := 0
	if endpointsRaw, exists := config["endpoints"]; exists {
		if endpointsArray, ok := endpointsRaw.([]any); ok {
			var newEndpoints []any
			for _, epRaw := range endpointsArray {
				epMap, ok := epRaw.(map[string]any)
				if !ok {
					newEndpoints = append(newEndpoints, epRaw)
					continue
				}
				if tag, ok := epMap["tag"].(string); ok && tag != "" && hasSubscriptionBracket(tag) {
					removedFromEndpoints++
					continue
				}
				newEndpoints = append(newEndpoints, epRaw)
			}
	...
	logger.Debug("清理所有订阅残留: %s (outbounds 移除 %d 个，endpoints 移除 %d 个)",
		configPath, removedCount, removedFromEndpoints)
	return nil
}
