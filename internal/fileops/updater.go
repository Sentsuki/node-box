...
// UpdateSelectorWithRelayNodes updates the selector with relay nodes based on relay_nodes configuration.
// This method adds relay node tags to the selector's outbounds array.
func (u *Updater) UpdateSelectorWithRelayNodes(configPath string, relayNodes []map[string]any, relayNodeNames []string) error {
	// 读取配置文件
	data, err := os.ReadFile(configPath)
	if err != nil {
		return fmt.Errorf("%w %s: %v", ErrConfigFileRead, configPath, err)
	...
	for _, node := range relayNodes {
		if tag, ok := node["tag"].(string); ok {
			// 检查节点是否匹配指定的 relay 节点名称
			for _, relayName := range relayNodeNames {
				if strings.Contains(strings.ToLower(tag), strings.ToLower(relayName)) {
					relayTags = append(relayTags, tag)
					break
				}
			}
		}
	...
	for i, outboundRaw := range outboundsArray {
		if outboundMap, ok := outboundRaw.(map[string]any); ok {
			if tag, ok := outboundMap["tag"].(string); ok && tag == u.insertMarker {
				...
				if outboundList, ok := outboundMap["outbounds"].([]any); ok {
					// 添加 relay 节点标签
					for _, relayTag := range relayTags {
						// 检查是否已存在，避免重复添加
						exists := false
						for _, existingTag := range outboundList {
							if existingTagStr, ok := existingTag.(string); ok && existingTagStr == relayTag {
								exists = true
								break
							}
						}
						if !exists {
							outboundList = append(outboundList, relayTag)
						}
					}
					outboundMap["outbounds"] = outboundList
					log.Printf("添加 relay 节点到 selector %s: %d 个标签", u.insertMarker, len(relayTags))
				} else {
					// 如果outbounds字段不存在，直接设置为relay节点标签数组
					var newOutboundList []any
					for _, relayTag := range relayTags {
						newOutboundList = append(newOutboundList, relayTag)
					}
					outboundMap["outbounds"] = newOutboundList
					log.Printf("创建 selector %s 并添加 relay 节点: %d 个标签", u.insertMarker, len(relayTags))
				}
