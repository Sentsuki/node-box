...
// CleanAllSubscriptionArtifacts removes ALL nodes (outbounds) and selector tags
...
func (u *Updater) CleanAllSubscriptionArtifacts(configPath string) error {
	...
	for _, outboundRaw := range outboundsArray {
		...
		// 如果是带有 tag 的节点并且匹配 [xxx] 模式，整条节点删除
		if tag, ok := outboundMap["tag"].(string); ok && tag != "" {
	...
	logger.Debug("清理所有订阅残留: %s (移除 %d 个节点)", configPath, removedCount)
	return nil
}
