...
Relay 是 node-box 的高级功能，用于构建链式代理（多跳转发）。整体流程分三步：
...
### 2. `include_relay` 全局筛选

从全部展开节点中，根据 `include_relay` 规则筛选出要写入配置文件的真实节点。

### 3. `relay_nodes` 逐 selector 筛选

在每个 `proxies[]` 项中，`relay_nodes` 作为 include 关键词列表，决定哪些已写入的 relay 节点 tag 出现在该 selector 的 outbounds 中。
...
| `no_need` | string[] | ❌ | 后处理关键词列表。合成后，outbounds 和 endpoints 中 tag 包含这些关键词的节点会被移除 |
